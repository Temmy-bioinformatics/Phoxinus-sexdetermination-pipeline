...
rule difcover:
    ...
    output:
        unionbed_file = os.path.join(config["OUTPUT_DIR"], "difcover", "{species}", "sample1_sample2.unionbedcv"),
    ...
    shell:
        ...
        cd $(dirname {output.ratio_file})
        

	# Run pre-step: unionbedcv
        {config[SCRIPTS_DIR]}/from_bams_to_unionbed.sh {input.female_bam} {input.male_bam}
